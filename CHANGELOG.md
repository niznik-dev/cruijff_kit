...
- `--cpus_per_task` knob in `setup_finetune.py` (also readable from `setup_finetune.yaml` and `controls.cpus_per_task` in `experiment_summary.yaml`). Overrides `MODEL_CONFIGS[model]["slurm"]["cpus"]` for the generated `#SBATCH --cpus-per-task=`. **When to use:** workloads with long sequences (near `max_seq_len`) or heavy preprocessing (rich chat templates, large per-sample transforms) — CPU work per batch scales with these, so extra cores can produce a small GPU-util bump via tokenizer threading and OpenMP even at `num_workers=0`. Short-text experiments are GPU-dominated and won't benefit; leave it unset. Does *not* parallelize data loading itself (that's recipe-level, blocked by the wrapper-only design principle; see #465). If a value helps consistently across the workloads a given model is used for, the principled follow-up is a `MODEL_CONFIGS` default update with measured evidence, not permanent reliance on the override. (#449)
