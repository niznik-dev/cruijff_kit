...
- `evaluation.max_connections` field in `experiment_summary.yaml`, plumbed through `scaffold-inspect` → `eval_config.yaml` → `setup_inspect.py` as `--max-connections N` on the generated `inspect eval` command. Default is **128** (was inspect-ai's upstream default of 32) — better suited to 1B-3B local HF models on 40-80GB GPUs. Note that this is a *ceiling* on samples per HF batch, not the effective batch size: inspect-ai's HF provider also fires batches on a hardcoded 2s timeout when sample arrival is slow, so raising the ceiling does not always translate into larger batches. (#318)
