...
- **Per-task and per-run prompt overrides**: the user `prompt` can now vary, not just at the experiment-wide `controls.prompt`. Set `evaluation.tasks[].prompt` for an eval-side prompt sweep, or `runs[].parameters.prompt` to give a fine-tune its own prompt across both training and its eval cells. Precedence: per-task > per-run > `controls.prompt`. Prompts are now checked for the `{input}` placeholder and fail loudly if it's missing (`str.format` would otherwise silently drop the input). (#562)
...
- **`experiment_summary.yaml` section-membership principle written down** (`docs/ARCHITECTURE.md`): a field's home is decided by granularity × role, with a duplication-vs-independence test (`system_prompt` collapses; `seed` stays per-stage). (#562)
