...
- Adapter-only saves with self-loading offline — handled in the wrapper layer by `src/tools/torchtune/post_finetune.py`, which rewrites `adapter_config.json`'s base path after the SLURM training step finishes (`port_cruijff_adapter` restores the HF Hub name for export)
- Validation during training (requires nightly build)

Per the Recipe Patching Policy (#465), no in-`train()` patches live in the custom recipes; behaviors like the post-save adapter handling are wrapper-layer steps invoked from the SLURM script.
...
- **save_adapter_weights_only**: `'true'`/`'false'` (default `'true'`) - Save only the LoRA adapter (~MB), skip the merged base+LoRA checkpoint (~GB). The post-finetune step (`src/tools/torchtune/post_finetune.py`, invoked from the SLURM script after training) rewrites the saved `adapter_config.json`'s `base_model_name_or_path` to the local base-model path so the dir is self-loading on offline compute. Use `python -m cruijff_kit.tools.torchtune.port_cruijff_adapter <epoch_dir>` to restore the HF Hub name when exporting.
