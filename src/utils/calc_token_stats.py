...
parser = argparse.ArgumentParser(
    description="Process a JSON file to calculate token statistics."
)
parser.add_argument(
    "--json_filename",
    type=str,
    help="Name of the input file (should be in the same directory as this script).",
)
parser.add_argument(
    "--model_path",
    type=str,
    required=True,
    help="Path to the model directory (e.g. /path/to/pretrained-llms/Llama-3.2-1B-Instruct)",
)
...
for entry in data:
    ...
    if key == "1-1":
        print(
            f"Input: {input}, Tokens: {tokenizer.convert_ids_to_tokens(input_tokens_clean['input_ids'][0])}"
        )
        print(
            f"Output: {output}, Tokens: {tokenizer.convert_ids_to_tokens(output_tokens_clean['input_ids'][0])}"
        )
...
print(
    f"{percentage_different_length:.2f}% of words have different token lengths between input and output."
)
