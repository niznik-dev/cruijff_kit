...
Example usage:
    ...
    report = generate_report(
        ...
        output_path=Path("experiments/acs_income/exploration/report.md")
    )
"""
...
def _format_model_table(
    ...
    Args:
        eval_split_name: Name of the dataset split used for evaluation
            (e.g., "eval", "test"). Shown in the sample-size footnote.
    ...
    for m in sorted_metrics:
        epoch_str = (
            str(int(m.epoch)) if m.epoch is not None and not pd.isna(m.epoch) else "-"
        )
...
def _format_visualizations(
    ...
    Args:
        ...
        generated_pngs: Optional list of PNG paths to embed. If None, embeds
            all PNGs found in the exploration directory (legacy behavior).
    """
    if generated_pngs is not None:
        png_files = sorted(generated_pngs)
    else:
        # Legacy fallback: glob all PNGs in directory
        exploration_dir = output_path.parent
        png_files = sorted(exploration_dir.glob("*.png"))
...
def _format_inspect_view_commands(
    ...
    if len(log_dirs) > max_commands:
        lines.append(
            "<pre><code>inspect view start --log-dir &lt;LOG_DIR&gt;</code></pre>"
        )
        lines.append("")
        lines.append(
        ...
        commands = "\n".join(f"inspect view start --log-dir {d}" for d in log_dirs)
...
def generate_report(
    ...
    compute_section: Optional[str] = None,
) -> str:
    """
    ...
    Args:
        ...
        compute_section: Optional pre-formatted markdown for compute utilization.
            When provided, inserted after "Exploration & Interpretation" section.
    ...
    Example:
        report = generate_report(
            ...
            generated_pngs=[Path("exploration/scores_by_task.png")],
            eval_log_paths=[Path("logs/log1.eval"), Path("logs/log2.eval")],
            generated_by="Claude Opus 4.6",
            compute_section="## Compute Utilization\\n...",
        )
    """
    # Extract metrics
    ...
    if future_directions:
        report_lines.append("## Exploration & Interpretation\n")
        report_lines.append(
            "*This section is generated by the explore-experiment skill based on the results above.*\n\n"
        )
        report_lines.append(future_directions + "\n")

    # Add compute utilization section if provided
    if compute_section:
        report_lines.append(compute_section + "\n")

    report_lines.append("---\n")

    # Provenance footer
    attribution = (
        f"Generated by {generated_by} via explore-experiment skill"
        if generated_by
        else "Generated by explore-experiment skill"
    )
