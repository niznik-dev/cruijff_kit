...
def compute_training_steps(
    training_samples, batch_size, gradient_accumulation_steps, epochs
):
...
def warn_on_low_steps(step_info, num_warmup_steps):
    ...
    print(
        f"Training step summary: {step_info['steps_per_epoch']} steps/epoch, "
...
def create_parser():
    ...
    parser.add_argument(
        "--config_file",
        type=str,
        default="setup_finetune.yaml",
        help="Path to YAML configuration file. Values from this file will be used as defaults, and can be overridden by CLI arguments.",
    )
    ...
    parser.add_argument(
        "--my_wandb_project",
        type=str,
        default="PredictingZygosity",
        help="Project for when results are synced to wandb",
    )
    parser.add_argument(
        "--my_wandb_run_name",
        type=str,
        help="Name for when results are synced to wandb; if not provided, a random name will be generated",
    )
    parser.add_argument(
        "--input_formatting",
        type=str,
    ...
    parser.add_argument(
        "--max_steps_per_epoch",
        type=int,
        help="Maximum steps per epoch (useful for debugging)",
    )
    parser.add_argument(
        "--log_every_n_steps", type=int, default=5, help="How often to log (in steps)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--input_key", type=str, default="input", help="JSON key for input field"
    )
    parser.add_argument(
        "--output_key", type=str, default="output", help="JSON key for output field"
    )
    parser.add_argument(
        "--system_prompt",
        type=str,
        default="",
        help="System prompt (legacy, for instruct_dataset)",
    )
    parser.add_argument(
        "--train_on_input",
        type=parse_bool,
        default=False,
        help="Whether to train on the input data (true/false)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="LoRA rank (alpha will be auto-calculated as 2*rank)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Learning rate scheduler function name (without 'torchtune.training.lr_schedulers.' prefix)",
    )
    parser.add_argument(
        "--dataset_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=parse_bool,
        default=False,
        help="Whether to use packed sequences (true/false). Should be False for custom datasets.",
    )
    parser.add_argument(
        "--max_seq_len",
        type=int,
    ...
    # ------ Slurm Args -----
    parser.add_argument(
        "--time", type=str, default="00:15:00", help="Time to run the job (HH:MM:SS)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to the virtual environment to use (if not using conda)",
    )
    parser.add_argument(
        "--modules",
        type=str,
        default="",
        help="Modules to load before running the script, separated by commas (e.g. '2024,Python/3.12.3')",
    )
    ...
    parser.add_argument(
        "--custom_recipe",
        type=str,
        help="Full name of a custom recipe file in the repo's custom_recipes folder to use for fine-tuning",
    )
    parser.add_argument(
        "--base_recipe",
        type=str,
...
def main():
    ...
    # same parsing that CLI values receive (e.g. parse_bool).
    _type_converters = {}
    ...
    for key, value in config_data.items():
        ...
        if hasattr(args, key):
            ...
            if current_value == default_value:
                ...
                # and a converter exists (e.g. parse_bool)
    ...
    model_run_name = (
        args.my_wandb_run_name if args.my_wandb_run_name else RANDOM_MODEL_RUN_NAME
    )
    ...
    for key, value in vars(args).items():
        ...
        # Special cases first
            ...
            config["lr_scheduler"]["_component_"] = (
                f"torchtune.training.lr_schedulers.{value}"
            )
    ...
    if gpus > 1:
        slurm_script = slurm_script.replace(
            "#SBATCH --gres=gpu:1", "#SBATCH --gres=gpu:" + str(gpus)
        )
        slurm_script = slurm_script.replace(
            "lora_finetune_single_device",
            "--nproc_per_node=" + str(gpus) + " lora_finetune_distributed",
        )
    slurm_script = slurm_script.replace(
        "#SBATCH --cpus-per-task=1", "#SBATCH --cpus-per-task=" + str(cpus)
    )

    if args.account:
        slurm_script = slurm_script.replace(
            "##SBATCH --account=<ACT>", "#SBATCH --account=" + args.account
        )
    ...
    if partition is not None and partition != "":
        slurm_script = slurm_script.replace(
            "##SBATCH --partition=<PART>", "#SBATCH --partition=" + partition
        )
    ...
    if constraint:
        slurm_script = slurm_script.replace(
            "##SBATCH --constraint=<CONST>", "#SBATCH --constraint=" + constraint
        )
    if args.custom_recipe:
        ...
        if gpus == 1:
            ...
            slurm_script = slurm_script.replace(
                "lora_finetune_distributed", custom_recipe + ".__main__"
            )
    ...
    if args.venv:
        slurm_script = slurm_script.replace(
            ...
            f"source $PROJECT/venvs/{args.venv}/bin/activate",
        )
    if args.modules:
        slurm_script = "\n".join(
            line for line in slurm_script.splitlines() if "conda" not in line
        )
        module_string = ""
