...
- emit the canonical 4-line log block that explore's regex consumes,
...
The regex in .claude/skills/explore/generation.md is
...
def log_monitor_detached(log_path: Path, reason: str, summary: dict) -> None:
    ...
    this only records that the watcher exited. explore's
...
def log_monitor_config(
    ...
    Block shape (no `Job ID:` line — safe from the explore
...
def log_oom_retry(
    ...
    Block shape (no `Job ID:` line — safe from the explore regex,
