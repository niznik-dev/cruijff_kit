"""
Eval task for ACS mobility prediction with chat_completion dataset.

Reads prompt and system_prompt from setup_finetune.yaml to ensure train/eval parity.

Usage:
    inspect eval inspect_task_acs_mobility.py --model hf/local \
        -M model_path=/path/to/checkpoint \
        -T config_path=/path/to/setup_finetune.yaml \
        -T data_path=/path/to/acs_mobility_condensed_50000_80P.json
"""
...
def acs_mobility(
    ...
    """
    Eval task for ACS mobility prediction (moved in last year) trained with chat_completion.
