...
def cap_task(config_path: str) -> Task:
    ...
    try:
        ...
        DATA_PATH = (
            setup_finetune["input_dir_base"]
            + setup_finetune["dataset_label"]
            + (
                ".json"
                if (setup_finetune["dataset_ext"] == ".json" and not USE_CHAT_TEMPLATE)
                else "/test.json"
            )
        )
    ...
    if USE_JSON_FORMAT:
        ...
        if USE_CHAT_TEMPLATE:
            ...
            dataset = json_dataset(DATA_PATH, record_to_sample)
        else:
            ...
            def record_to_sample(record, prompt_template=EVAL_PROMPT):
                ...
                if "{input}" in prompt_template:
                    formatted_input = prompt_template.replace(
                        "{input}", record["input"]
                    )
                else:
    ...
    return Task(
        ...
        solver=chain(
            ...
            generate(temperature=1e-7, max_tokens=10),
        ),
