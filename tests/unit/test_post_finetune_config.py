...
OmegaConf ${...} interpolation (e.g. checkpoint_dir: ${models_dir}/...). The
recipe-internal version resolved these via torchtune's OmegaConf load; an early
wrapper used yaml.safe_load and silently wrote a literal "${models_dir}" into
...
def _write_finetune_yaml(tmp_path: Path, **overrides) -> Path:
    """Write a minimal finetune.yaml that uses ${models_dir} interpolation."""
    cfg_path = tmp_path / "finetune.yaml"
    save_only = overrides.get("save_adapter_weights_only", True)
    cfg_path.write_text(
        "models_dir: /scratch/pretrained-llms\n"
...
def test_resolves_models_dir_interpolation(tmp_path):
    ...
    _output_dir, base_model_path, _save_only = read_run_config(cfg_path)

    # The interpolation must be resolved — not left as a literal "${models_dir}".
...
def test_save_adapter_weights_only_defaults_false_when_absent(tmp_path):
    ...
    _output_dir, _base, save_only = read_run_config(cfg_path)
