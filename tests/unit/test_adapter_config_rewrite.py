...
def _write_epoch(tmp_path: Path, epoch: int, *, with_repo_id: bool = True) -> Path:
    epoch_directory = tmp_path / f"epoch_{epoch}"
    epoch_directory.mkdir(parents=True)
    (epoch_directory / "adapter_config.json").write_text(
        json.dumps(
            {
                "r": 8,
                "lora_alpha": 16,
    ...
    if with_repo_id:
        (epoch_directory / "original_repo_id.json").write_text(
            json.dumps({"repo_id": "meta-llama/Llama-3.2-1B-Instruct"})
        )
    return epoch_directory


def _read_base(epoch_directory: Path) -> str:
    return json.loads((epoch_directory / "adapter_config.json").read_text())[
...
def test_port_cruijff_restores_hub_name_single_epoch_directory(tmp_path):
    epoch_directory = _write_epoch(tmp_path, 0)
    # Simulate a cruijff-rewritten config first
    rewrite_adapter_config_base_path(
        str(tmp_path), 0, "/local/abs/path", _silent_logger()
    )
    assert _read_base(epoch_directory) == "/local/abs/path"

    port_cruijff_adapter.restore_one(epoch_directory)

    assert _read_base(epoch_directory) == "meta-llama/Llama-3.2-1B-Instruct"


def test_port_cruijff_skips_when_no_original_repo_id_and_no_override(tmp_path, capsys):
    epoch_directory = _write_epoch(tmp_path, 0, with_repo_id=False)
    rewrite_adapter_config_base_path(
        str(tmp_path), 0, "/local/abs/path", _silent_logger()
    )

    changed = port_cruijff_adapter.restore_one(epoch_directory)
...
def test_port_cruijff_uses_repo_id_override_when_marker_missing(tmp_path):
    ...
    changed = port_cruijff_adapter.restore_one(
        epoch_directory, repo_id_override="meta-llama/Llama-3.2-1B-Instruct"
    )
...
def test_port_cruijff_repo_id_override_wins_over_marker(tmp_path):
    epoch_directory = _write_epoch(tmp_path, 0, with_repo_id=True)
    rewrite_adapter_config_base_path(
        str(tmp_path), 0, "/local/abs/path", _silent_logger()
    )

    port_cruijff_adapter.restore_one(
        epoch_directory, repo_id_override="my-org/forked-model"
    )

    assert _read_base(epoch_directory) == "my-org/forked-model"


def test_port_cruijff_recurses_over_run_directory(tmp_path, monkeypatch, capsys):
    _write_epoch(tmp_path, 0)
...
def test_check_returns_none_when_base_path_exists(tmp_path):
    ...
    assert check_adapter_base_path(epoch_directory) is None
...
def test_check_flags_missing_local_base_path(tmp_path):
    ...
    problem = check_adapter_base_path(epoch_directory)
...
def _write_merged_save(tmp_path: Path, epoch: int) -> Path:
    ...
    epoch_directory = tmp_path / f"epoch_{epoch}"
    epoch_directory.mkdir(parents=True)
    (epoch_directory / "model.safetensors").write_text("fake-merged-weights")
    (epoch_directory / "adapter_config.json").write_text("{}")
    (epoch_directory / "adapter_model.safetensors").write_text("fake-adapter-weights")
    return epoch_directory


def test_stash_moves_adapter_files_into_subdir(tmp_path):
    epoch_directory = _write_merged_save(tmp_path, 0)
    stash_adapter_files(str(tmp_path), 0, _silent_logger())

    assert (epoch_directory / "model.safetensors").exists()  # merged stays
    assert not (epoch_directory / "adapter_config.json").exists()  # adapter moved
    assert not (epoch_directory / "adapter_model.safetensors").exists()
    assert (epoch_directory / "adapter_weights" / "adapter_config.json").exists()
    assert (epoch_directory / "adapter_weights" / "adapter_model.safetensors").exists()
...
def test_stash_skips_missing_files_gracefully(tmp_path):
    ...
    epoch_directory = tmp_path / "epoch_0"
    epoch_directory.mkdir()
    (epoch_directory / "model.safetensors").write_text("merged")
    (epoch_directory / "adapter_config.json").write_text("{}")
    ...
    assert (epoch_directory / "adapter_weights" / "adapter_config.json").exists()
    assert not (
        epoch_directory / "adapter_weights" / "adapter_model.safetensors"
    ).exists()
