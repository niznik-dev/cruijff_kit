...
def temp_run_directory(tmp_path):
    ...
    # The script uses script_directory / "templates" so we need to run from a dir
    # where that path resolves correctly, OR we can symlink

    run_directory = tmp_path / "test_run"
    run_directory.mkdir()

    yield run_directory


def run_setup_finetune(
    run_directory, model_name="Llama-3.2-1B-Instruct", extra_args=None
):
    ...
    slurm_file = run_directory / "finetune.slurm"
...
class TestModelAwareSlurmResources:
    ...
    def test_1b_default_memory(self, temp_run_directory):
        """1B model defaults to MODEL_CONFIGS memory."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.2-1B-Instruct")
        expected_mem = MODEL_CONFIGS["Llama-3.2-1B-Instruct"]["slurm"]["mem"]
        assert f"#SBATCH --mem={expected_mem}" in slurm

    def test_1b_no_partition(self, temp_run_directory):
    ...
    def test_1b_no_constraint_by_default(self, temp_run_directory):
        """1B model without CLI constraint leaves line commented."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.2-1B-Instruct")
        assert "##SBATCH --constraint=<CONST>" in slurm

    def test_1b_constraint_via_cli(self, temp_run_directory):
    ...
    def test_3b_default_memory(self, temp_run_directory):
        """3B model defaults to MODEL_CONFIGS memory."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.2-3B-Instruct")
        expected_mem = MODEL_CONFIGS["Llama-3.2-3B-Instruct"]["slurm"]["mem"]
        assert f"#SBATCH --mem={expected_mem}" in slurm

    def test_3b_no_constraint_by_default(self, temp_run_directory):
        """3B model without CLI constraint leaves line commented."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.2-3B-Instruct")
        assert "##SBATCH --constraint=<CONST>" in slurm

    def test_3b_no_partition(self, temp_run_directory):
        """3B model without CLI partition leaves line commented."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.2-3B-Instruct")
        assert "##SBATCH --partition=<PART>" in slurm

    def test_8b_default_memory(self, temp_run_directory):
        """8B model defaults to MODEL_CONFIGS memory."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.1-8B-Instruct")
        expected_mem = MODEL_CONFIGS["Llama-3.1-8B-Instruct"]["slurm"]["mem"]
        assert f"#SBATCH --mem={expected_mem}" in slurm

    def test_8b_no_constraint_by_default(self, temp_run_directory):
        """8B model without CLI constraint leaves line commented."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.1-8B-Instruct")
        assert "##SBATCH --constraint=<CONST>" in slurm

    def test_8b_single_gpu(self, temp_run_directory):
    ...
    def test_70b_default_memory(self, temp_run_directory):
        """70B model defaults to MODEL_CONFIGS memory."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.3-70B-Instruct")
        expected_mem = MODEL_CONFIGS["Llama-3.3-70B-Instruct"]["slurm"]["mem"]
        assert f"#SBATCH --mem={expected_mem}" in slurm

    def test_70b_no_constraint_by_default(self, temp_run_directory):
        """70B model without CLI constraint leaves line commented."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.3-70B-Instruct")
        assert "##SBATCH --constraint=<CONST>" in slurm

    def test_70b_no_partition(self, temp_run_directory):
        """70B model without CLI partition leaves line commented."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.3-70B-Instruct")
        assert "##SBATCH --partition=<PART>" in slurm

    def test_70b_multi_gpu(self, temp_run_directory):
        """70B model defaults to MODEL_CONFIGS GPU count."""
        slurm = run_setup_finetune(temp_run_directory, "Llama-3.3-70B-Instruct")
        expected_gpus = MODEL_CONFIGS["Llama-3.3-70B-Instruct"]["slurm"]["gpus"]
        assert f"#SBATCH --gres=gpu:{expected_gpus}" in slurm

    def test_70b_distributed_training(self, temp_run_directory):
...
class TestMigSupport:
    ...
    def test_mig_partition_stays_commented(self, temp_run_directory):
    ...
    def test_mig_enabled_reduced_memory(self, temp_run_directory):
...
class TestUserOverrides:
    ...
    def test_mem_override(self, temp_run_directory):
        """Custom --mem overrides model default."""
        slurm = run_setup_finetune(
            temp_run_directory, "Llama-3.2-1B-Instruct", extra_args=["--mem", "64G"]
        )
        assert "#SBATCH --mem=64G" in slurm
        assert "#SBATCH --mem=40G" not in slurm

    def test_partition_override(self, temp_run_directory):
    ...
    def test_constraint_override(self, temp_run_directory):
...
class TestCpuAllocation:
    ...
    def test_1b_default_cpus(self, temp_run_directory):
