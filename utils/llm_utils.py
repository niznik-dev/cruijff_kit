...
def load_model(
    ...
    base_model = AutoModelForCausalLM.from_pretrained(
        model_path, device_map="auto", low_cpu_mem_usage=True, torch_dtype=torch_dtype
    )
    ...
    if adapter_path is not None:
        ...
        model_out = PeftModel.from_pretrained(
...
def tokenize_prompts(
    ...
    if use_chat_template:
        ...
        inputs = tokenizer(
            flat_prompts, return_tensors="pt", padding=padding, max_length=max_length
        )
    return inputs
...
def get_logits(
    ...
    **kwargs,
    ...
    loader = torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, collate_fn=collate, pin_memory=True
    )
    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        for batch_inputs in tqdm(loader):
            ...
            input_lengths = (
                batch_inputs["attention_mask"].sum(dim=1) - 1
            )  # subtract 1 for zero-based indexing

            batch_logits = outputs.logits[
                torch.arange(len_batch), input_lengths
    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        for batch_inputs in tqdm(loader):
            ...
            if num_return_sequences > 1:
                batch_generated_tokens = batch_generated_tokens.reshape(
                    len_batch, num_return_sequences, -1
                )
            ...
            if generated_tokens is None:
                ...
                generated_tokens = torch.cat(
                    (generated_tokens, batch_generated_tokens), dim=0
                )
    ...
    **kwargs,
    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        for batch_inputs in tqdm(loader):
            ...
            batch_pooled_embed = pool_hidden_states(
                batch_embed, pool=pool, attention_mask=batch_mask
            )
...
def pool_hidden_states(
    hidden_states: torch.Tensor, pool: str = "last", attention_mask: torch.Tensor = None
) -> torch.Tensor:
    """
    Pool hidden states over the token dimension.
    ...
    with torch.no_grad():
        if pool == "mean":
            ...
            assert attention_mask is not None, (
                "attention_mask is required for pool = 'mean_non_padding' "
            )
            ...
            pooled = torch.median(hidden_states, axis=2)[
                0
            ]  # Get only median values, not indices
            ...
            assert attention_mask is not None, (
                "attention_mask is required for pool = 'last_non_padding' "
            )

            input_lengths = attention_mask.sum(dim=1) - 1
            pooled = hidden_states[
                torch.arange(hidden_states.shape[0]), :, input_lengths
            ]
...
def save_tensor_with_ids(
    ...
    if attention_mask is not None:
        assert len(tensor) == len(attention_mask), (
            "Length of tensor and attention_mask must match"
        )
    ...
    logger.info(
        f"Saved tensor of shape {tensor.shape}{', attention_mask' if attention_mask is not None else ''} and ids to {filename}"
    )


def load_tensor_with_ids(
    filename, dataset_name="tensor", ids_name="ids_dim0"
    ...
    logger.info(
        f"Loaded tensor of shape {tensor.shape}{', attention_mask' if attention_mask is not None else ''} and ids from {filename}"
    )
