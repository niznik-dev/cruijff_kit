...
4. **Handle multiple base runs with different system prompts:**
   - Create separate `eval_config.yaml` for each run with its specific system prompt
   - Example: `Llama-3.2-1B-Instruct_base_helpful/eval_config.yaml` vs `Llama-3.2-1B-Instruct_base_concise/eval_config.yaml`
