...
**Case B — User has pre-existing separate files per condition.** The user already has files like `dict_full_train_s42.json`, `dict_synonym_test_s42.json`, generated by the convert-tabular-to-text skill. Record the paths in each run's `parameters` as `dataset_path` (training) and `eval_dataset_path` (evaluation). Include the `data_generation` section in experiment_summary.yaml for documentation only — no generation step needed.
...
Record these in the `data_generation` section of `experiment_summary.yaml` (see template). This section is documentation only — it does not trigger generation. Pre-populate the runs list with the expected file paths following the convention `{scratch_dir}/ck-data/generated/{condition_name}_{split}_s{seed}.json`.

**Advise:** "After we finalize the experiment design, you'll run the `convert-tabular-to-text` skill to generate the actual datasets before scaffolding."

#### Per-Run Dataset Overrides (Cases B and C)

When different runs use different dataset files, each run's `parameters` should include:
- `dataset_path` — full path to the training dataset file
- `eval_dataset_path` — full path to the evaluation dataset file

The scaffold agents check for these overrides and use them instead of `data.training.path` when present. The existing `data.training` section should still be populated (use the baseline training dataset as the default) so backward compatibility is preserved.
