...
Create structured configuration in `{experiment_directory}/experiment_summary.yaml` using the template structure from `templates/experiment_summary.yaml`.
...
- `controls`: parameters held constant across all runs, including `dataset_type` (`"chat_completion"` | `"text_completion"`, required)
- `models`: base model(s) with paths and sizes
- `data`: training dataset with splits
- `data.data_generation` (optional): declare a dataset to be generated by `scaffold-experiment` (set `tool: model_organism` — see `references/model_organisms.md`)
- `output`: checkpoint directories and naming patterns
- `runs`: complete list of runs — fine-tuned, control, and/or eval-only (eval-only runs carry `parameters.checkpoint_path`); optional `compute` block per run
- `evaluation`: system prompt, do_sample (optional, default false = greedy argmax), temperature (optional, required only when do_sample: true), scorers (structured list with optional params), tasks, matrix (with optional `compute` block)
...
Create human-readable audit trail in `{experiment_directory}/logs/design-experiment.log` that records all verification steps, calculations, and decisions.
...
- `{run_directory}/setup_finetune.yaml` - Fine-tuning config (by scaffold-torchtune)
- `{run_directory}/eval/{cell_name}/eval_config.yaml` - Per-cell eval config (one per (task, epoch); by scaffold-inspect)
- `{run_directory}/eval/{cell_name}/cell.slurm` - Per-cell evaluation script (by scaffold-inspect)
...
```
I've created the experiment plan at `{experiment_directory}/experiment_summary.yaml`.

All verification steps and calculations have been logged in `{experiment_directory}/logs/design-experiment.log`.
```
...
- Run `explore-experiment` skill to interpret results
...
1. **Create evaluation task:** Run `create-inspect-task` to create the capitalization task script
   ...
   - Expected location: `{repo_directory}/blueprints/capitalization/inspect_task.py`
