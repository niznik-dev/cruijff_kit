...
**Required sections:**
- `experiment`: name, question, date, hypothesis (optional), purpose (optional), dir
...
- `{run_dir}/eval/{cell_name}/eval_config.yaml` - Per-cell eval config (one per (task, epoch); by scaffold-inspect)
- `{run_dir}/eval/{cell_name}/cell.slurm` - Per-cell evaluation script (by scaffold-inspect)
...
1. **Create evaluation task:** Run `create-inspect-task` to create the capitalization task script
   ...
   - Expected location: `{repo_dir}/blueprints/capitalization/inspect_task.py`
