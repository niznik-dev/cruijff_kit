...
- `evaluation`: system prompt, do_sample (optional, default false = greedy argmax), temperature (optional, required only when do_sample: true), scorer (structured list with optional params), tasks, matrix (with optional `compute` block)
