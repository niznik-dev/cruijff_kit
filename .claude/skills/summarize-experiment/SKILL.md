...
- Fine-tuning: Check for `{experiment_directory}/{run_name}/artifacts/` and SLURM outputs
- Evaluation: Check for `{run_directory}/eval/*/logs/*.eval` files
...
1. Find SLURM stdout in the **output directory**:
   - Read `experiment.directory` from experiment_summary.yaml as `experiment_directory`
   - Look in: `{experiment_directory}/{run_name}/artifacts/slurm-*.out`
...
1. Find .eval files: `{run_directory}/eval/*/logs/*.eval`
...
1. **Find SLURM output files** in the eval directory: `{run_directory}/eval/slurm-*.out`
...
3. **Query job names via sacct:**
   ...
   ```
4. **Parse epoch from the job name** — scaffold-inspect names eval jobs `eval-{task}-ep{N}` (or `eval-{task}` for a base run with no epoch). The *run* is identified by which `{run_directory}/eval/` the slurm file sits in, not by the job name:
...
Create `{experiment_directory}/summary.md` with the following structure:
...
1. `{experiment_directory}/{run_name}/setup_finetune.yaml` → `input_dir_base` + `dataset_label` + `dataset_ext` = training dataset path for that run. This is authoritative — it's what torchtune actually loaded.
2. `{experiment_directory}/{run_name}/eval/*/eval_config.yaml` (per-cell — one config per (task, epoch); see issue #498) → test dataset path for that evaluation. Equivalently, the inspect `cell.slurm` script's `DATA_PATH`.
...
2. Export raw data: `inspect log export {run_directory}/eval/*/logs/*.eval --format csv`
...
Document the process in `{experiment_directory}/logs/summarize-experiment.log`.
