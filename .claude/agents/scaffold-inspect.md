...
These values are baked directly into the SLURM script - no intermediate config file needed.

### Handling Multiple Base Runs with Different System Prompts

When experiment_summary.md includes multiple base runs with different system prompts, each SLURM script gets its specific values:
...
For fine-tuned models, scaffold-inspect reads `setup_finetune.yaml` from the run directory and extracts the values to pass directly:
