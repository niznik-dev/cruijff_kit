...
- `eval_condition` - Name of a condition in `data.data_generation.conditions`. Scaffold resolves it to `{condition}_test_{hash8}.json` via `tabular_to_text_gen.lib.config_hash.resolve_dataset_path(data["data_generation"], eval_condition, "test", f"{scratch_dir}/ck-data/generated")`. Preferred when the eval file was generated by convert-tabular-to-text for this experiment.
...
- `data_path`: resolve in this order, stopping at the first match:
  1. If the evaluation task has `eval_condition` set — call `tabular_to_text_gen.lib.config_hash.resolve_dataset_path(data["data_generation"], task["eval_condition"], "test", f"{scratch_dir}/ck-data/generated")` and use the returned path.
