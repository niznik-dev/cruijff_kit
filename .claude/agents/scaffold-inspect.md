...
tasks:
  - name: "capitalization"
    ...
    # prompt: "..."            # Overrides controls.prompt (per-task prompt sweep)
    # system_prompt: "..."     # Overrides controls.system_prompt
...
- `prompt` (optional) - Per-task user-prompt template (with `{input}`). When present, the cells produced from this task use this prompt instead of the experiment-wide `controls.prompt`. This is the eval-side lever for a **prompt sweep**: one task script, N tasks differing only by an appended sentence (the canonical zero-finetuning prompt-engineering pattern). Same override semantics as `system_prompt`.
- `system_prompt` (optional) - Per-task system prompt. When present, the cells produced from this task use this prompt instead of `controls.system_prompt`. Lets two cells in the same run carry different prompts (the canonical example: a cue vs. no-cue ablation).
...
- Prompt overrides (`prompt`) — write *after* propagation when either the `evaluation.tasks[]` entry sets a per-task `prompt` (a prompt sweep) or the run sets `parameters.prompt` (carried into its eval cells for train/eval parity). Per-task wins over per-run.
...
- `prompt` *(override only)*: write this **only** to override the
  experiment-wide `controls.prompt` the helper already propagated. Two
  sources, in precedence order:
  1. **Per-task** — the `evaluation.tasks[]` entry sets a task-level
     `prompt` (a prompt sweep). This wins.
  2. **Per-run** — the run's `parameters.prompt` is set. Carry it into the
     run's eval cells so a fine-tune is evaluated with the same prompt it
     trained on (train/eval parity). A per-task `prompt` still wins over it.

  Do not "copy" `controls.prompt` here — Step 1's helper already did that;
  only write `prompt` when one of the two overrides above applies.
- `system_prompt` *(per-task override only)*: write this **only** if the
  ...
  default the helper propagated. Do not "copy" `controls.system_prompt`
