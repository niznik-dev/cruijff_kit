...
2. **Generate eval_config.yaml** with all experiment-specific configuration (see full schema below).

3. **Call `setup_inspect.py`** to render the SLURM script from the template (see setup_inspect.py section below).
...
- `data_path`: **Check for per-run override first.** If `eval_dataset_path` exists in the run's `parameters` (from experiment_summary.yaml), use that path. Otherwise, fall back to the dataset path in setup_finetune.yaml (fine-tuned runs) or `data.training.path` in experiment_summary.yaml (control runs). This override enables experiments where different runs evaluate on different generated datasets.
...
After writing `eval_config.yaml`, render the SLURM script:
