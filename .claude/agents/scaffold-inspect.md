...
- `eval_condition` - Name of a condition in `data.data_generation.conditions`. Scaffold resolves it to `{condition}_test_{hash8}.json` via `cruijff_kit.tabular_to_text_gen.lib.config_hash.resolve_dataset_path(data["data_generation"], eval_condition, "test", f"{scratch_directory}/ck-data/generated")`. Preferred when the eval file was generated by convert-tabular-to-text for this experiment.
...
- Fine-tuned model path: `{experiment_directory}/{run_name}/artifacts/epoch_{N}`
...
- `model_path` — Fine-tuned: `{experiment_directory}/{run_name}/artifacts/epoch_{N}`. Control: `models.base[].path`. Eval-only: `parameters.checkpoint_path` verbatim (a pre-existing checkpoint trained outside this experiment).
...
1. **Create the cell directory + logs dir for every (task, epoch) pair:**
   ...
   mkdir -p {experiment_directory}/Llama-3.2-1B-Instruct_control/eval/capitalization/logs

   # Example: fine-tuned run "rank4" evaluated on capitalization at epoch 0 →
   mkdir -p {experiment_directory}/rank4/eval/capitalization_epoch0/logs

   # Example: same fine-tuned run evaluated on TWO prompt variants of the
   # same task (per-task system_prompt override) →
   mkdir -p {experiment_directory}/rank4/eval/acs_income_cue_epoch0/logs
   mkdir -p {experiment_directory}/rank4/eval/acs_income_nocue_epoch0/logs
   ```
...
Note: `config_path` and `eval_directory` are **auto-derived** from the location of the YAML file — do not include them in eval_config.yaml.
...
- `model_path`: Fine-tuned: `{experiment_directory}/{run_name}/artifacts/epoch_{N}`. Control: original model path. Eval-only: `parameters.checkpoint_path` verbatim (a pre-existing checkpoint; if it is somehow absent, fail loudly — do not silently fall back to the base model).
- `model_hf_name`: Fine-tuned: `hf/{run_name}_epoch_{N}`. Control: `hf/{run_name}_control`. Eval-only: `hf/{run_name}`
- `output_dir`: `{experiment_directory}/{run_name}/artifacts/`
- `data_path`: resolve in this order, stopping at the first match:
  1. If the evaluation task has `eval_condition` set — call `cruijff_kit.tabular_to_text_gen.lib.config_hash.resolve_dataset_path(data["data_generation"], task["eval_condition"], "test", f"{scratch_directory}/ck-data/generated")` and use the returned path.
...
```bash
cd {run_directory}/eval/{cell_name}
python {cruijff_kit_path}/src/tools/inspect/setup_inspect.py \
  --config eval_config.yaml \
  --model_name "Llama-3.2-1B-Instruct" \
  --time "0:10:00" \
...
- `cd` to eval_directory (the cell directory) before running inspect
...
After `setup_inspect.py` writes the SLURM script, spot-check that `GPU_METRICS_DIRECTORY` in the rendered file resolves to a path containing `/artifacts/epoch_N` for fine-tuned runs. If it doesn't, `model_path` is malformed (missing `/artifacts/epoch_N` segment) and should be fixed in eval_config.yaml.
...
```
{experiment_directory}/{run_directory}_control/
└── eval/
    └── {task_name}/
        ├── eval_config.yaml
        ├── cell.slurm
...
```
{experiment_directory}/{run_directory}/eval/
...
**Output location:**
- Cell directory: `{run_directory}/eval/{cell_name}/`
- Cell slurm: `{run_directory}/eval/{cell_name}/cell.slurm`
- inspect-ai log directory: `{run_directory}/eval/{cell_name}/logs/`
- SLURM stdout (`slurm-{job_id}.out`): lands with the GPU-metrics directory — `{run_directory}/artifacts/epoch_N/` for fine-tuned cells, `{run_directory}/artifacts/` for base/control cells. Not inside the cell dir.
...
```bash
OUTPUT_BASE="{experiment_directory}/{run_name}/artifacts"
MODEL_PATH="$OUTPUT_BASE/epoch_0"
CONFIG_PATH="{experiment_directory}/{run_directory}/eval/{cell_name}/eval_config.yaml"
```
...
MODEL_PATH="$OUTPUT_BASE/epoch_0"
CONFIG_PATH="{experiment_directory}/{run_directory}/eval/{cell_name}/eval_config.yaml"
DATA_PATH="{ck_data_directory}/capitalization/words_5L_80P_1000.json"
...
- `config_path` points to `eval_config.yaml` (generated from experiment_summary.yaml and stored in `{run_directory}/eval/`), which the task reads at runtime
...
CONFIG_PATH="{experiment_directory}/{run_directory}/eval/{cell_name}/eval_config.yaml"
DATA_PATH="{ck_data_directory}/acs_income/income_5feat_test.json"
...
**Key points:**
- `model_path` is `parameters.checkpoint_path` verbatim — do not recompute it from `{experiment_directory}`. If `checkpoint_path` is missing on an eval-only run, fail loudly.
...
mkdir -p {experiment_directory}/{run_directory}/eval/{cell_name}/logs

# Write eval_config.yaml into the cell directory:
cat > {experiment_directory}/{run_directory}/eval/{cell_name}/eval_config.yaml << 'EOF'
{config content}
EOF
...
- Check if task file has `config_directory` parameter (preferred for experiments)
- Fall back to `dataset_path` + `system_prompt` approach
- Log the decision

## Logging

Create a detailed log file at `{experiment_directory}/logs/scaffold-inspect.log`:
...
Note: Task supports config_directory parameter for experiment integration

[2025-10-24 17:00:20] CREATE_CELL_DIRECTORY: rank8_lr1e-5/eval/capitalization_epoch0
...
Details: Fine-tuned model evaluation with config_directory integration
...
2. Results will be written to `{run_directory}/eval/{cell_name}/logs/` directories — one logs/ per cell
...
- Evaluation logs will be written to `{run_directory}/eval/{cell_name}/logs/` subdirectories (one logs/ per cell)
