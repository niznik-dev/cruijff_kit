...
- Place in `{experiment_dir}/{run_dir}/eval/eval_config.yaml`
...
1. **Create directories for all run evaluation:**
   ```bash
   ...
   # Example for fine-tuned run named "rank4"
   mkdir -p {experiment_dir}/rank4/eval
   mkdir -p {experiment_dir}/rank4/eval/logs
   ```

2. **Generate eval_config.yaml** with content extracted from experiment_summary.yaml (control) or setup_finetune.yaml (fine-tuned):
   ```yaml
   # Evaluation configuration for run
...
3. **Extract values from setup_finetune.yaml (fine-tuned runs) or experiment_summary.yaml (control runs):**
...
- `config_path`: Path to `eval_config.yaml` containing prompt/system_prompt and scorer configuration (same for fine-tuned and control models)
...
- Copy `prompt` and `system_prompt` to `eval_config.yaml`
- Add scorer configuration from `evaluation.scorer` in experiment_summary.yaml

**Writing eval_config.yaml for all runs (fine-tuned and control):**
Generate `eval_config.yaml` for every run with:
1. Prompt and system_prompt (from `setup_finetune.yaml` for fine-tuned runs, from experiment_summary.yaml for control runs)
2. Scorer list from `evaluation.scorer` in experiment_summary.yaml
3. This allows task files to read scorer configuration at runtime and instantiate scorers dynamically (e.g., `risk_scorer(option_tokens=["0", "1"])`)

**Extracting values for control models:**
Use values from experiment_summary.yaml Configuration section to populate `eval_config.yaml`.
...
- All eval SLURM scripts point `config_path` to `eval_config.yaml` in the eval directory
