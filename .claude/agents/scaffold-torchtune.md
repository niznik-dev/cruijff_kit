...
3. **Control hyperparameters — non-propagated only:**
   ...
   **All other `controls.*` fields** (`epochs`, `batch_size`, `batch_size_val`, `gradient_accumulation_steps`, `weight_decay`, `lora_dropout`, `prompt`, `system_prompt`) are populated by `propagate_train_fields()` (see "Key Pattern: Propagate First"). Don't extract them here — the helper reads them straight from `experiment_summary.yaml`. The helper also resolves and writes the training `seed` (`controls.seed`, else default `14`). The eval seed is independent (`evaluation.seed`, else `14`); both default to 14, so they match unless set differently.
