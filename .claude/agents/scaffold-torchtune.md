...
3. **Control hyperparameters — non-propagated only:**
   ...
   **All other `controls.*` fields** (`epochs`, `batch_size`, `batch_size_val`, `gradient_accumulation_steps`, `weight_decay`, `lora_dropout`, `prompt`, `system_prompt`, `seed`) are populated by `propagate_train_fields()` (see "Key Pattern: Propagate First"). Don't extract them here — the helper reads them straight from `experiment_summary.yaml`.
