...
def load_data(file_path, group_column="GROUP"):
    ...
    if group_column != "GROUP":
        if group_column not in df.columns:
            raise ValueError(
                f"Group column '{group_column}' not found in data columns: {df.columns.tolist()}"
            )
        df["GROUP"] = df[group_column]
    elif "GROUP" not in df.columns:
        raise ValueError(
            "Default 'GROUP' column not found and no alternative group column specified."
        )
...
def calculate_group_metrics(df):
    ...
    for group in df["GROUP"].unique():
        ...
        accuracy = accuracy_score(group_df["TRUE_LABEL"], group_df["PREDICTION"])

        try:
            auc = roc_auc_score(group_df["TRUE_LABEL"], group_df["P(TRUE LABEL)"])
        except ValueError:
            auc = np.nan

        metrics[group] = {"accuracy": accuracy, "auc": auc, "n_samples": len(group_df)}
            ...
            if std_auc > 0:
                ...
                if z_score_auc < z_threshold or metrics["auc"] < auc_threshold:
                    identified_groups["auc_outliers"].append(
                        {"group": group, "auc": metrics["auc"], "z_score": z_score_auc}
                    )
                    auc_outlier = True
            elif metrics["auc"] < auc_threshold:
                identified_groups["auc_outliers"].append(
                    {"group": group, "auc": metrics["auc"], "z_score": 0}
                )
                auc_outlier = True
...
def visualize_data(group_metrics, output_dir="results"):
    ...
    # AUC plot
    aucs = [
        group_metrics_str[g]["auc"] if not np.isnan(group_metrics_str[g]["auc"]) else 0
...
def generate_report(
    df, group_metrics, heterogeneity_results, identified_groups, output_dir="results"
):
    ...
    report = {
        "summary": {
            ...
            "heterogeneity_found": bool(
                heterogeneity_results["accuracy_heterogeneity"]["heterogeneity"]
                or heterogeneity_results["auc_heterogeneity"]["heterogeneity"]
            ),
        },
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analyze model predictions for heterogeneity"
    )
